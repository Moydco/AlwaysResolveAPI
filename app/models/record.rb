class Record
  ...
  def unique_name?
    if self.type == 'SOA'
      ...
      errors.add(:routing_policy, 'Already have a resource with this name that conflicts with this routing policy') unless self.domain.records.where(:name => self.name, :type => 'CNAME').not_in(:routing_policy => self.routing_policy).count == 0
      # if already exists a CNAME record with the same name but with simple routing policy
      errors.add(:routing_policy, 'Already have a resource with this name that conflicts with this routing policy') unless self.domain.records.where(:name => self.name, :type => 'CNAME', :routing_policy => 'SIMPLE').count == 0
      ...
      # Checks for routing_policy field
      # if already exists a CNAME record with the same name but with different routing policy
      errors.add(:routing_policy, 'Already have a resource with this name that conflicts with this routing policy') unless self.domain.records.where(:name => self.name, :type => self.type).not_in(:routing_policy => self.routing_policy).count == 0
      # if already exists a CNAME record with the same name but with simple routing policy
      errors.add(:routing_policy, 'Already have a resource with this name that conflicts with this routing policy') unless self.domain.records.where(:name => self.name, :type => self.type, :routing_policy => 'SIMPLE').count == 0
      # if already exist a failover primary/secondary record and I request to be primary/secondary
      if self.routing_policy == 'FAILOVER'
        if self.primary
